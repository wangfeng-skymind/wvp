	...
	*
	...
	public void playStreamCmd(Device device, String channelId, ZLMHttpHookSubscribe.Event event, SipSubscribe.Event errorEvent) {
		try {
			...
			System.out.println("=ss=xx=1===发送命令：：" +ssrc + "----------"+ streamId +">>>>"+ streamMode);
...
//			content.append("o="+channelId+" 0 0 IN IP4 "+mediaInfo.getWanIp()+"\r\n");
			...
			if (seniorSdp) {
				if("TCP-PASSIVE".equals(streamMode)) {
					...
					//System.out.println("===4===发送命令：：" +content.toString());

				}else if ("TCP-ACTIVE".equals(streamMode)) {
					content.append("m=video "+ mediaPort +" TCP/RTP/AVP 96 126 125 99 34 98 97\r\n");
					//System.out.println("===5===发送命令：：" +content.toString());
				}else if("UDP".equals(streamMode)) {
					content.append("m=video "+ mediaPort +" RTP/AVP 96 126 125 99 34 98 97\r\n");
					//System.out.println("===6===发送命令：：" +content.toString());
				}
				//System.out.println("===7===发送命令：：" +streamMode);
				...
				//	System.out.println("===8===发送命令：：" +content.toString());

				}else if ("TCP-ACTIVE".equals(streamMode)) {
					content.append("m=video "+ mediaPort +" TCP/RTP/AVP 96 98 97\r\n");
				//	System.out.println("===9===发送命令：：" +content.toString());
				}else if("UDP".equals(streamMode)) {
					content.append("m=video "+ mediaPort +" RTP/AVP 96 98 97\r\n");
					//System.out.println("===10===发送命令：：" +content.toString());
				}
			//	System.out.println("==xx=11===发送命令：：" +streamMode);
				...
				if("TCP-PASSIVE".equals(streamMode)) { // tcp被动模式
					...
					content.append("a=rtpmap:98 H264/90000\r\n");
					content.append("a=rtpmap:97 MPEG4/90000\r\n");
				///	System.out.println("==xx=12===发送命令：：" +streamMode);

					if ("TCP-PASSIVE".equals(streamMode)) { // tcp被动模式
						content.append("a=setup:passive\r\n");
						content.append("a=connection:new\r\n");
					//	System.out.println("==xx=13===发送命令：：" +streamMode);

					} else if ("TCP-ACTIVE".equals(streamMode)) { // tcp主动模式
					} else if ("TCP-ACTIVE".equals(streamMode)) { // tcp主动模式
						content.append("a=setup:active\r\n");
						content.append("a=connection:new\r\n");
					//	System.out.println("==xx=14===发送命令：：" +streamMode);
		...
		//	System.out.println("=xxx=====final====15====" + content.toString());
			String tm = Long.toString(System.currentTimeMillis());
	...
	public boolean screenshotByeCmd(Device nvrDevice, String channelId, String uploadUrl) {
		...
		try {
			...
			cmdXml.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
			cmdXml.append("<Control>\r\n");
			cmdXml.append("<CmdType>DeviceControl</CmdType>\r\n");
			...
			// SnapShotConfig 节点
			cmdXml.append("<SnapShotConfig>\r\n");
			cmdXml.append("<ChannelID>").append(channelId).append("</ChannelID>\r\n"); // NVR 通道 ID
			cmdXml.append("<SnapNum>1</SnapNum>\r\n"); // 抓拍 1 张
			cmdXml.append("<Interval>0</Interval>\r\n"); // 立即抓拍
			cmdXml.append("<UploadURL>").append(uploadUrl).append("</UploadURL>\r\n"); // 上传地址
			cmdXml.append("<SessionID>").append(UUID.randomUUID().toString()).append("</SessionID>\r\n");
			cmdXml.append("<SnapType>0</SnapType>\r\n"); // 类型，0=手动
			cmdXml.append("</SnapShotConfig>\r\n");
